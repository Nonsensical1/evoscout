...
async function fetchLiveData(topicsMap: any = {}, newsLimit: number = 12) {
  ...
  try {
    ...
    if (freshNewsCount < newsLimit) {
      console.log(`[News Pipeline] Quota uncompleted (${freshNewsCount}/${newsLimit}). Fetching PubMed fallback...`);
      try {
        ...
        if (searchRes.ok) {
          ...
          if (pmids.length > 0) {
            ...
            if (summaryRes.ok) {
              ...
              allNews = allNews.concat(pubmedNews);
            }
          }
        }
...
Jobs: ${JSON.stringify(aiPayload)}`;
  ...
  try {
    ...
    // Increase chunk size to reduce the number of sequential API requests
    const batches = chunkArray(summarizedItems, 35);
    const masterSummaryDict: any = {};

    // Sleep to prevent rate limit collision if the Careers Gemini call was made immediately before
    if (didCallCareersGemini && batches.length > 0) {
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      try {
        ...
        for (let attempt = 0; attempt < retries; attempt++) {
            ...
            if (gRes.ok) break;
            if (gRes.status === 429) {
                console.warn(`[Gemini] Batch summarization hit 429. Backing off ${6000 + attempt * 2000}ms...`);
